...
export async function POST(req: Request, { params }: RouteParams) {
	...
	const normalizedBody = {
		selectedFileIds: Array.isArray(
			(body as { selected_asset_ids?: unknown }).selected_asset_ids,
		)
			? ((body as { selected_asset_ids: string[] }).selected_asset_ids ?? [])
			: (body as { selectedFileIds?: string[] }).selectedFileIds,
		templateVariantId: (body as { templateVariantId?: string })
			.templateVariantId,
		globalOverrides:
			(body as { global_overrides?: Record<string, string> })
				.global_overrides ??
			(body as { globalOverrides?: Record<string, string> }).globalOverrides,
		useAi:
			(body as { use_ai?: boolean }).use_ai ??
			(body as { useAi?: boolean }).useAi,
	};
	const parsed = PlanManualGenerationRequestSchema.safeParse(normalizedBody);
	if (!parsed.success) {
	...
	const planResponse = await fetchRag(
		`/api/v1/handbooks/${encodeURIComponent(manualId)}/plan`,
		{
			method: 'POST',
	...
	const assetsResponse = await fetchRag(
		`/api/v1/handbooks/${encodeURIComponent(manualId)}/assets?role=TEMPLATE`,
	);
	const assetsPayload = await safeJson(assetsResponse);
	const assets = (
		(assetsPayload as { assets?: RagAssetPayload[] }).assets ?? []
	...
	const outputTree = outputs.flatMap((entry, index) => {
		const relPath = String(
			(entry as { output_rel_path?: string }).output_rel_path ??
				`outputs/file-${index + 1}`,
		);
	...
	const manualPlan = {
		id: String(
			(planPayload as { run?: { id?: string } }).run?.id ?? randomUUID(),
		),
		manualId,
