...
export async function GET(_req: Request, { params }: RouteParams) {
	...
	const response = await fetchRag(
		`/api/v1/handbooks/${encodeURIComponent(manualId)}/runs/${encodeURIComponent(runId)}`,
	);
	...
	const summary =
		(metrics.summary as Record<string, unknown> | undefined) ?? {};
	const files = Array.isArray(metrics.files)
		? (metrics.files as Record<string, unknown>[])
		: [];
